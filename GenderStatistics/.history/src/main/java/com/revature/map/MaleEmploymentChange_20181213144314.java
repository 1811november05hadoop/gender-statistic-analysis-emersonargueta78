...
public class MaleEmploymentChange extends Mapper<LongWritable, Text, Text, Text> {
    final private static String fisrtLevelPattern = new Regex().getRegexes()[0];
    ...
    protected void map(LongWritable key, Text value, Context context)
        ...
        int[] yearRange = {MaleEmployment.lowerYearRange,MaleEmployment.upperYearRange};
        
        if(
            //true
            //!stringRow.toString().contains("female") &&
            stringRow.toString().contains("Employment") &&
            //stringRow.toString().contains("male")
            stringRow.toString().matches(fisrtLevelPattern)
          ){
    ...
    private void getCountryAndYearData(List<String> colValsList,
     ...
     throws IOException, InterruptedException {
        ...
        for (int year = rangeOfYears[0]; year < rangeOfYears[1]; year++) {
            ...
            double yearlyData = 0.0;
            if(!colValsList.get(dataColsYear.get(String.valueOf(year))).isEmpty()){
                yearlyData =  Double.valueOf(colValsList.get(dataColsYear.get(String.valueOf(year))));
            }
