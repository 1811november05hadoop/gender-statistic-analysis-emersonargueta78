...
public class CodeForFemaleEducation extends Mapper<LongWritable, Text, Text, Text> {
    ...
    private void getSeriesCode(List<String> colValsList, Context context)
     throws IOException, InterruptedException {
        ...
        Map<String, Integer> mapSeries = new GenderStatsCols().getSeriesCols();
        //Getting the series code
        String seriesCode = colValsList.get(
                                            mapSeries.get("Series Code")
                                            )
                                        .trim();
        
        Map<String, Integer> mapData = new GenderStatsCols().getSeriesCols();
        if(!seriesCode.contains(" ")){
            String indicatorName = colValsList.get(
                                                   mapData.get("Indicator Name")
                                                  );
