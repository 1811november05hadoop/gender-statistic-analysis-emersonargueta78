...
public class CodeForFemaleGraduation extends Mapper<LongWritable, Text, Text, Text> {
    final private static String searchEducationFemaleCodes = new Regex().getRegexes()[0];//"(.*)Educational(.*).*(.*)completed(.*).*(.*)female(.*)";
    final private static String searchEducationCodes = new Regex().getRegexes()[0];//"(.*)education(.*)";
